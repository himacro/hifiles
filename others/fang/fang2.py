...
class Houses(object):
    ...
    def dump_json(obj, fn):
        with open(fn, 'w') as f:
            json.dump(obj, f, indent=4, ensure_ascii=True, default=serialize)
    ...
    def report(self):
        ...
        for title, lst in table:
            ...
            if lst:
                for house in lst:
                    print(u'{h.hid}: {h.community} - {h.area}平|{h.layout} - {h.title} - {h.href}'.format(h=house))
                    for date, price in house.latest_prices(2).items():
#                   for date in house.latest_dates(days=2):
#                       if date in house.prices:
                        print(u'  {}:  {p[total_price]}万 / {p[unit_price]}'.format(date, p=price))
            else:
                print(u'无')
...
def parse_hwj(soup, date):
    ...
    for hl in house_lis:
        ...
        title = unicode(tag_title.a.string)
        path = tag_title.a['href']
        hid = 'HWJ_' + os.path.basename(path)
        href = 'http://dl.hwj.com'+path

        community = unicode(tag_a1.find(class_='m_r1_lou').string).strip()
        area = float(unicode(tag_a1.find(class_='m_r1_mj').string).strip()[:-2])
        layout = unicode(tag_a1.find(class_='m_r1_hx').string.strip())
        floor = ""

        total_price = float(unicode(tag_a1.find(class_='m_r1_price').string).strip()[:-1])
        unit_price = float(unicode(tag_a1.find(class_='m_r1_dj').string).strip()[:-3])
...
def parse_lj(soup, date):
    ...
    for hl in house_lis:
        ...
        community = unicode(col1.find(class_='region').string.strip())
        area = float(unicode(col1.find(class_='meters').string).strip()[:-2])
        layout = unicode(col1.find(class_='zone').string.strip())
        floor = unicode(next(col1.find(class_='con').strings).strip())

        total_price = float(unicode(col3.find(class_='price').find('span').string).strip())
        unit_price = float(unicode(col3.find(class_='price-pre').string).strip()[:-4])

        tag = col1.find(class_='fang05-ex')
        school = unicode(tag.string.strip()) if tag else ""
...
def update_houses(json_fn):
    lj_community_urls = (
        u'http://dl.lianjia.com/xiaoqu/1311042052472/esf/',
        u'http://dl.lianjia.com/xiaoqu/1311042053235/esf/',
        u'http://dl.lianjia.com/xiaoqu/1311042053299/esf/',
        u'http://dl.lianjia.com/xiaoqu/1311042053230/esf/',
        u'http://dl.lianjia.com/xiaoqu/1311042053238/esf/',
        u'http://dl.lianjia.com/xiaoqu/1311042053231/esf/'
        )

    hwj_community_urls = (
        u'http://dl.hwj.com/search/ershou?project=老年公寓',
        u'http://dl.hwj.com/search/ershou?project=学清园',
        u'http://dl.hwj.com/search/ershou?project=学子园',
        u'http://dl.hwj.com/search/ershou?project=知音园',
        u'http://dl.hwj.com/search/ershou?project=双语学校',
        u'http://dl.hwj.com/search/ershou?project=学院派'
        )
